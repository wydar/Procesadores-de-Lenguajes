...
public class Generator{
  ...
  public static void multiArrayRangeCheck(String var,String [] dims, int nDim,String temp){
      Node nod =TS.getNode(var);
      int [] tams = nod.getMoreDimTam();
  /*    if(tams.lenght!=nDim){
        TS.error(4);
      }*/
      for(int i=0;i<nDim;i++){
        ...
        out.println("   if ("+dims[i]+" < 0) goto "+dt.getV()+";");
        out.println("   if ("+tams[i]+" < "+dims[i]+") goto "+dt.getV()+";");
        out.println("   if ("+tams[i]+" == "+dims[i]+") goto "+dt.getV()+";");
        GenGoto(dt.getF());
        GenLabel(dt.getV());
        out.println("   error;");
        out.println("   halt;");
        Generator.GenLabel(dt.getF());
      }
  }

  public static String GenUseVarMultiArray(String var,String [] dims, int nDim, String temp,boolean isAsig){
      multiArrayRangeCheck(var,dims,nDim,temp);
      int type= varType(var);
      temps[getNumTemp(temp)].setType(type);
      int [] tams = TS.getNode(var).getMoreDimTam();
      String aux ="";
      for(int i=0;i<nDim-1;i++){
        if(i==0){
          GenBinOper(Integer.toString(tams[i+1]),POR,dims[i],temp);
          GenBinOper(temp,MAS,dims[i+1],temp);
        }else{
          aux = newTag("");
          temps[getNumTemp(aux)].setType(type);
          GenBinOper(Integer.toString(tams[i+1]),POR,temp,aux);
          GenBinOper(aux,MAS,dims[i+1],aux);
          temp=aux;
        }
      ...
      temps[getNumTemp(aux)].setType(type);
      if(!isAsig){
        out.println("   "+aux+" = "+var+"["+temp+"];");
      }
      return aux;
  }

  public static void GenAsigMultiArray(String id,String temp,String exp){
    out.println("   "+id+"["+temp+"] = "+exp+";");
  }
